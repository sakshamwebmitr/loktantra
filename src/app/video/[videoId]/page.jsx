import { getVideoNews } from "@/utils/post";
...
const videoPage = async ({ params }) => {
  // console.log(params)
  const [post] = await getVideoNews(params.videoId);
  // console.log(post);
  ...
  return (
    ...
    <meta property="og:type" content="article" />
        ...
        <meta
          property="og:url"
          content={`https://www.loktantraudghosh.com/video/${post?.id}`}
        />
    </head>
    <div className="flex flex-col items-center  p-4 mx-4 my-8 bg-white border sm:ml-16 rounded-xl border-slate-200">
      ...
      <h5 className="text-xl md:mx-6 md:w-50" >{post.x_author}</h5> <h6 className="md: w-50">{post.x_date}</h6>
      </div>
        </div> */}
        {/* <div className="flex items-center justify-center p-4 m-2 rounded w-50">
            <div className="flex items-center gap-4">
             
              <a
                target="_blank"
                href={`https://www.facebook.com/sharer.php?u=https://www.loktantraudghosh.com/video/${post?.id}}`}
              >
                <i className="text-3xl text-blue-600 ri-facebook-circle-fill"></i>
              </a>
              <a
                target="_blank"
                href={`https://twitter.com/intent/tweet?text=${post?.title} https://www.loktantraudghosh.com/video/${post?.id} `}
              >
                <i className="text-3xl ri-twitter-x-fill"></i>
              </a>
              <a
                target="_blank"
                href={`https://api.whatsapp.com/send?text=${`*${post?.title}*https://www.loktantraudghosh.com/video/${post?.id}`}`}
              >
...
          </div> */}
      ...
      <div className="flex items-center justify-center w-full px-10 m-2 border w-50 md:p-4 rounded-xl">
        <div className="flex items-center gap-4">
          ...
          <a
            target="_blank"
            href={`https://t.me/share/url?url=https://www.loktantraudghosh.com/video/${post?.id}&text=${post?.title}`}
          >
